...
const medicalPersonnelFilters = [
    ...
    <ReferenceInput source="id" label="Vet personnel" reference="vetPersonnel" allowEmpty>
...
export const MedicalPersonnelCreate = props => {
    ...
    const { data: animalsChoices } = useQuery({
        type:'getList',
        resource: 'animals',
        payload: {
            pagination: { page: 1, perPage: 600 },
    ...
    useEffect(() => {
        ...
        if(animalsChoices) setanimals(animalsChoices.map((item) => ({ id:item.id, name:item.description })))
    }, [catalogueOfServicesChoices])

    return (
    <Create onSuccess={onSuccess} {...props} title='Create new vet personnel' onSuccess={onSuccess}>
        <SimpleForm>             
            ...
            source="listOfServices" choices={catalogueOfServices} />
            <AutocompleteArrayInput 
            parse={value =>
                value && value.map(v => ({ id: v }))
            }
            format={value => value && value.map(v => v.id)}            
            source="animals" choices={animals} />
        </SimpleForm>
    </Create>
)}